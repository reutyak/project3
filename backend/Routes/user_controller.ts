...
import { checkJWT, getExpFromJWT, getJWT, getUserIdFromJWT, getUserNameFromJWT } from '../Utils/jwt';
...
//get all vacations
user_router.get("/vacation/all", async (request: Request, response: Response, next: NextFunction) => {
...
user_router.get("/vacation/single/:id", async (request: Request, response: Response, next: NextFunction) => {
  const someData = +request.params.id;
...
user_router.post("/login", async (request: Request, response: Response, next: NextFunction)=>{
  ...
  users.map(async item=>{
    if(detailsUser.typeUser==="user"&&  hash(detailsUser.user_name)===item.user_name && hash(detailsUser.password)===item.password){
      console.log(detailsUser.user_name);
      // sessionStorage.setItem('user',detailsUser.id);
