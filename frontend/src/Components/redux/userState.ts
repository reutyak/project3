...
export function vacationReducer(
    ...
    case vacationActionType.updateVacationSt:
        newState.vacationsSt = (newState.vacationsSt.filter((item: { id: any; })=> item.id !== action.payload)).push(action.payload);
    break;

    case vacationActionType.deleteVacationSt:
        console.log(newState);
        console.log(action.payload);
        newState.vacationsSt = newState.vacationsSt.filter((item: { id: any; })=> item.id !== action.payload);
        console.log(newState);

    break;

    // case vacationActionType.addFollow:
    //     const addLike=newState.vacationsSt.filter((item: { id: any;  })=> item.id !== action.payload)
    //     addLike.followed_list+=1;
    //     newState.vacationsSt=addLike.push(action.payload);
    // break;

    // case vacationActionType.reduceFollow:
    //     const reduceLike=newState.vacationsSt.filter((item: { id: any;  })=> item.id !== action.payload)
    //     reduceLike.followed_list-=1;
    //     newState.vacationsSt=reduceLike.push(action.payload);
    // break;
}
